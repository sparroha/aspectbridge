...
export async function GET(req: Request, context: any, res: Response) {
    ...
    const [newregistry] = await sql`SELECT * FROM aspect_registry_ WHERE name = ${name};`
    if (!newregistry) return NextResponse.json({ final: 'no registry created' })
    return NextResponse.json({ data: newregistry, sqlresponse: newR, create: create, query: query })
}