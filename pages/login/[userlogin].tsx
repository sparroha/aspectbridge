...
export function ProfileByIp({ip, setUser}) {
  ...
  if (error) {setUser({username: ('guest'+ip.split(".")[3]), email: null, access: 0});return <Row><Col style={{visibility: (debug?'visible':'hidden'), position: (debug?'relative':'absolute')}}>{JSON.stringify(error)}:User not cached. Please login or register.</Col></Row>}
  if (!data) return <div>loading...</div>
  else {
    let {username, email, access} = data
    data.message = 'Welcome back '+data.username+'!'
