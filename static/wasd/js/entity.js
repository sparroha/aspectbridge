...
function newProjectile(entity,caster,target,maxlife,size,speed){
	...
	vecObjs.push(new vecObj(entity(caster.attr('id'),startX,startY,size||1,angle),new vec(stepX,stepY),speed,life,decay));
}
function missile(id,startx,starty,size,angle)
{
...
function fireball(id,startx,starty,size,angle){
	this.collision = function(target){};
	var width=20*size;
	var height=20*size;
	angle=angle;
...
function icicle(id,startx,starty,size,angle){
	this.collision = function(target){};
	var width=20*size;
	var height=20*size;
	angle=angle+45;
